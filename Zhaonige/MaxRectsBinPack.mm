...
MyRect MaxRectsBinPack::Insert(int width, int height, FreeRectChoiceHeuristic method)
{
	MyRect newNode;
...
void MaxRectsBinPack::Insert(std::vector<RectSize> &rects, std::vector<MyRect> &dst, FreeRectChoiceHeuristic method)
{
	dst.clear();
	...
	while(rects.size() > 0)
	{
		...
		for(size_t i = 0; i < rects.size(); ++i)
		{
			int score1;
			int score2;
			MyRect newNode = ScoreRect(rects[i].width, rects[i].height, method, score1, score2);
...
void MaxRectsBinPack::PlaceRect(const MyRect &node)
{
	size_t numRectanglesToProcess = freeRectangles.size();
	for(size_t i = 0; i < numRectanglesToProcess; ++i)
	{
...
MyRect MaxRectsBinPack::ScoreRect(int width, int height, FreeRectChoiceHeuristic method, int &score1, int &score2) const
{
	MyRect newNode;
...
MyRect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestY, int &bestX) const
{
	MyRect bestNode;
	memset(&bestNode, 0, sizeof(MyRect));
...
MyRect MaxRectsBinPack::FindPositionForNewNodeBestShortSideFit(int width, int height, 
...
MyRect MaxRectsBinPack::FindPositionForNewNodeBestLongSideFit(int width, int height, 
	int &bestShortSideFit, int &bestLongSideFit) const
{
	MyRect bestNode;
	memset(&bestNode, 0, sizeof(MyRect));
...
MyRect MaxRectsBinPack::FindPositionForNewNodeBestAreaFit(int width, int height,
...
MyRect MaxRectsBinPack::FindPositionForNewNodeContactPoint(int width, int height, int &bestContactScore) const
{
	MyRect bestNode;
	memset(&bestNode, 0, sizeof(MyRect));
...
bool MaxRectsBinPack::SplitFreeNode(MyRect freeNode, const MyRect &usedNode)
{
